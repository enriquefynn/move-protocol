...
def plot_comparison(tput_paths, cross_shard, real_workload=False):
    ...
    for idx, p in enumerate(tput_paths):
        ...
        if idx == 0 and real_workload == False:
            ax.bar(idx, tput/(time/1e9), color=(0.2, 0.4, 0.6, 0.6),
                   label='{}% cross-shard'.format(cross_shard))
        else:
            ax.bar(idx, tput/(time/1e9), color=(0.2, 0.4, 0.6, 0.6))

        if idx > 0 and real_workload == False:
            if idx == 1:
                ax.bar(idx, base_tps[idx] - tput/(time/1e9), color='lightblue',
                       hatch='//', bottom=tput/(time/1e9), label='0% cross-shard')
            else:
                ax.bar(idx, base_tps[idx] - tput/(time/1e9),
                       color='lightblue', hatch='//', bottom=tput/(time/1e9))
